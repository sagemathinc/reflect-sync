...
describeIfCasefold(
  hasCasefoldRoot && hasNotCasefoldRoot
    ? "casefold integration"
    : "casefold integration [skipped: casefold fixtures unavailable]",
  () => {
    ...
    it("keeps conflicting directory trees isolated when syncing into a case-insensitive root", async () => {
      ...
      if (!canonicalFile) throw new Error("canonical directory is empty");
...
describeIfCasefold(
  hasCasefoldRoot && hasNotCasefoldRoot
    ? "casefold reverse integration"
    : "casefold reverse integration [skipped: casefold fixtures unavailable]",
  () => {
    let session: TestSession | undefined;
    let alphaRoot: string | undefined;
    let betaRoot: string | undefined;
