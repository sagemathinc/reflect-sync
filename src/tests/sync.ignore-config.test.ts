...
describe("planner ignores IGNORE_FILE itself", () => {
  let tmp: string;
  beforeAll(async () => {
    tmp = await fsp.mkdtemp(join(os.tmpdir(), "rfsync-ignore-config-"));
  });
  afterAll(async () => {
    await fsp.rm(tmp, { recursive: true, force: true });
  });

  test(`${IGNORE_FILE} is not copied alpha→beta`, async () => {
  ...
  test(`${IGNORE_FILE} on beta stays even if missing on alpha (no delete)`, async () => {
